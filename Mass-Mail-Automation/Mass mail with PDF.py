...
def send_emails():
    ...
    # Counters for successful and skipped emails
    success_count = 0
    skip_count = 0
    ...
    for name, email in zip(names, emails):
        ...
        try:
            ...
            # Handle SMTP exceptions (e.g., invalid email addresses)
            skip_count += 1
            continue  # Skip to the next email if there's an error
    
    # Show a message box indicating the emails have been sent
    tk.messagebox.showinfo("Emails sent", f"Successfully sent: {success_count}\n Skipped : {skip_count}\n")
...
body_label = tk.Label(body_frame, text="Email body:", width=20, font=labelfont, bg=lf_bg)
body_label.grid(row=0, column=0, padx=5, pady=5, sticky=W)

body_text = tk.Text(body_frame, width=125, height=15, font=entryfont)
